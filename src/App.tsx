...
export default function App(){const[splash,setSplash]=useState(true);if(splash)return <Splash onDone={()=>setSplash(false)}/>;return <div className="app-shell"><main className="content"><Routes><Route path="/" element={<DashboardPage/>}/><Route path="/treinar" element={<TrainPage/>}/><Route path="/calendario" element={<CalendarPage/>}/><Route path="/treinos" element={<WorkoutsPage/>}/><Route path="/treinos/prontos" element={<ReadyWorkoutsPage/>}/><Route path="/treinos/ia-import" element={<AiWorkoutImportPage/>}/><Route path="/perfil" element={<ProfilePage/>}/></Routes></main><nav className="bottom-nav"><NavLink to="/" end><LayoutDashboard size={20}/><span>Dashboard</span></NavLink><NavLink to="/treinar"><Activity size={20}/><span>Treinar</span></NavLink><NavLink to="/calendario"><CalendarDays size={20}/><span>Frequência</span></NavLink><NavLink to="/treinos"><Dumbbell size={20}/><span>Treinos</span></NavLink><NavLink to="/perfil"><UserRound size={20}/><span>Perfil</span></NavLink></nav></div>}
